...
<template>
    <div class="space-y-4">
        <n-card :bordered="false" class="panel-card panel-switch">
            ...
            <n-data-table :columns="columns" :data="slots" :loading="loading" :pagination="{ pageSize: 8 }"
                          :scroll-x="860" :single-line="false" class="mobile-table mt-4"
        ...
        <n-card :bordered="false" class="panel-card panel-switch">
            ...
            <n-alert :title="reserveResultText" :type="reserveResultType" class="mb-3" show-icon/>
            <div v-if="currentTraceId" class="flex flex-wrap gap-2">
                <n-button @click="queryReserveResult">手动刷新结果</n-button>
                <n-button v-if="reserveResult && reserveResult.status === 'PENDING'" @click="startPolling">
                    自动轮询结果
                </n-button>
            </div>
        </n-card>
